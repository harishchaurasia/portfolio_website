...
const ProjectCard: React.FC<ProjectCardProps> = ({ title, description, skills = [], mainSkill = '', skillIcons = [], githubRepo }) => {
  ...
  return (
    <a href={githubRepo} target="_blank" rel="noopener noreferrer" className="border border-gray-800 rounded-3xl shadow-bs p-4 flex flex-col md:flex-row items-start bg-black-800 bg-opacity-50 backdrop-blur-lg hover:shadow-xs text-white mb-4 no-underline">
      {mainSkill && <img src={mainSkillIconUrl} alt={`${mainSkill} logo`} className="w-16 h-16 mb-4 mr-4" />}
      <div className="flex-1">
        <div className="flex justify-between items-center">
          <h2 className="text-xl md:text-2xl font-bold">{title}</h2>
          <img src="./ex.png" alt="External link icon" className="absolute top-4 right-4 w-6 h-6 ml-2" />
        </div>
